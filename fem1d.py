...
class GlobalNode:
    ...
    def from_ints(ints: list[int]) -> list[GlobalNode]:
        return [GlobalNode(idx=i, is_boundary=False) for i in ints]
...
class BoundaryValueProblem1D:
    ...
    def __init__(self, op: BilinearForm, elements: Sequence[Element1D], f: ForcingFunc, boundary: Boundary1D):
        ...
        if boundary.btype == BoundaryType.Dirichlet:
    ...
    def result(self, x: float, u: Sequence[float]) -> float:
        if self.boundary.is_func_condition():
            u = [self.boundary.vals[0]] + list(u) + [self.boundary.vals[1]]
        for e in elements:
